...
package org.ebayopensource.webauthn.res;
...
import org.ebayopensource.webauthn.msg.AuthResponse;
...
import org.ebayopensource.webauthn.op.AuthProcessor;
import org.ebayopensource.webauthn.op.RegProcessor;
...
import org.ebayopensource.webauthn.stats.Dash;
...
public class FidoUafResource {
	...
	RegProcessor regProcessor = new RegProcessor();
	...
	@Produces(MediaType.APPLICATION_JSON)
	public String regRequest(String payload) {
		RegResponse response = new RegResponse();
		if (payload == null || payload.trim().length() == 0){
			response.processingStatus = "BAD_REQUEST";
			return regResponseJson(response);
		}
		
		RegRequest req = gson.fromJson(payload, RegRequest.class);
		if (req == null){
			response.processingStatus = "BAD_REQUEST_FORMAT";
			return regResponseJson(response);
		}
		
		response.challenge = regProcessor.genChallenge();
		response.processingStatus = "SUCCESS";
		return regResponseJson(response);
	}
	
	private String regResponseJson(RegResponse response) {
		Dash.getInstance().add(response);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public String reg(String payload) {
		if (payload == null || payload.trim().length() == 0){
			RegResponse response = new RegResponse();
			response.processingStatus = "BAD_REQUEST";
			return regResponseJson(response);
		...
		RegResponse response = gson.fromJson(payload, RegResponse.class);
		if (response == null){
		...
		try {
			return registrationRecordJson(regProcessor.reg(response, StorageImpl.getInstance()));
			...
			response.processingStatus = "DUPLICATE_KEY";
			return regResponseJson(response);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public String authRequest(String payload) {
		AuthResponse res = new AuthResponse();
		res.challenge = authProcessor.genChallenge();
	...
	public String auth(String payload) {
		...
		Assertion assertion = gson.fromJson(payload, Assertion.class);
		try {
			record = authProcessor.auth(assertion, StorageImpl.getInstance());
			if (record != null){
				record.processingStatus = "SUCCESS";
				return registrationRecordJson(record);
			} else {
				record = new RegistrationRecord();
				record.processingStatus = "AUTH_OP_PROCESSING_FAILED";
				return registrationRecordJson(record);
			}
		} catch (Exception e) {
			record = new RegistrationRecord();
			record.processingStatus = "AUTH_OP_FAILED";
			return registrationRecordJson(record);
		} 
	}

	private String registrationRecordJson(RegistrationRecord record) {
		record.processingTimeInMillis = System.currentTimeMillis();
		Dash.getInstance().add(record);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public String dereg(String payload) {
		return "{}";
	}
